extends RefCounted

const КодыМодулей = preload("res://геном/коды_модулей.gd")


static func прочитать(части: PackedStringArray) -> Dictionary:
	if части.size() != 2:
		return {
			"ошибка": "Ген трансформации должен иметь формат: 03,органика;"
		}

	var текст_органики: String = части[1].strip_edges()

	if not текст_органики.is_valid_float():
		return {
			"ошибка": "Некорректное количество органики трансформатора: '%s'" % текст_органики
		}

	var органика: float = текст_органики.to_float()

	if органика <= 0.0:
		return {
			"ошибка": "Органика трансформатора должна быть больше нуля."
		}

	return {
		"тип": КодыМодулей.ТРАНСФОРМАЦИЯ,
		"органика": органика
	}
